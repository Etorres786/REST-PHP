...
class Pedidos extends REST_Controller {
  ...
  public function realizar_orden_post($token = "0", $id_usuario = "0",$total="0"){
...
/*REVISAR TOTAL */
    ...
    $insertar = array('usuario_id' => $id_usuario,'TOTAL'=>$to );
    $this->db->insert( 'ordenes', $insertar );
    $orden_id = $this->db->insert_id();

    // crear el detalle de la orden
    $this->db->reset_query();
      foreach( $cantidad as $can => $v ){
        $data_insertar = array( 'producto_id' => $items[$can], 'orden_id'=>$orden_id,'valor'=>$valor[$can], 'cantidad' => $cantidad[$can], 'subtotal'=>$subtotal[$can]);
  ...
  public function obtener_pedidos_get($token = "0", $id_usuario ="0" ){
    ...
    foreach( $query->result() as $row ){
      ...
      $query_detalle = $this->db->query('SELECT a.orden_id,a.cantidad,a.subtotal,c.TOTAL,b.* FROM `ordenes_detalle` a inner join productos b on a.producto_id = b.codigo INNER JOIN ordenes c on c.id=a.orden_id where orden_id = '. $row->id );
